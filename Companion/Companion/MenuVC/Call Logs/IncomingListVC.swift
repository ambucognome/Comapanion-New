...
class IncomingListVC: UIViewController{
...
extension IncomingListVC : UITableViewDataSource, UITableViewDelegate {
    ...
    func numberOfSections(in tableView: UITableView) -> Int {
        return logData?.incoming.count ?? 0
    }
    
    func tableView(_ tableView: UITableView, titleForHeaderInSection section: Int) -> String? {
        let data = logData?.incoming[section]
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let data = logData?.incoming[indexPath.section]
