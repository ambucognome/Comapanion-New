...
open class ChatViewController: UIViewController, UITableViewDataSource, UITableViewDelegate, UITextViewDelegate, ImagePickerResultDelegate, DocumentPickerHelperResultDelegate {
    ...
    /// Document button
    @objc open func didPressDocButton(_ sender: Any?) {
        print("document")
        DocumentPickerHelper.shared.parentViewController = self
        DocumentPickerHelper.shared.accessPDF()
        DocumentPickerHelper.shared.delegate = self
//        switch currentKeyboardType {
//        case .none:
//            currentKeyboardType = .document
//            animateShowImagePicker()
//        case .default:
//            currentKeyboardType = .document
//            chatBarView.textView.resignFirstResponderTimeAnimate = 0.0
...
//        }
    ...
    public func didSelectPDF(url: URL?) {
...
extension ChatViewController {
    ...
    private func chatSlackBarStyle() {
        ...
        chatBarView.documentButton
            .configure { [weak self] in
                $0.size = CGSize(width: 32, height: 38)
                var image = UIImage(named: "ic_document", in: Bundle.chatBundle, compatibleWith: nil)
                if let tempImage = image?.withRenderingMode(.alwaysTemplate) {
                    image = tempImage
                    $0.image = image
                    $0.tintColor = self?.configuration.galleryButtonDeSelectedColor
            ...
            .onSelected {
                $0.image = UIImage(named: "ic_document", in: Bundle.chatBundle, compatibleWith: nil)
            }
            .onDeselected { [weak self] in
                var image = UIImage(named: "ic_document", in: Bundle.chatBundle, compatibleWith: nil)
                if let tempImage = image?.withRenderingMode(.alwaysTemplate) {
        ...
        chatBarView.documentButton.addTarget(self, action: #selector(didPressDocButton(_:)), for: .touchUpInside)
        ...
        let items = [.flexibleSpace, chatBarView.galleryButton, chatBarView.documentButton, chatBarView.sendButton]
    ...
    private func defaultChatBarStyle() {
        ...
        chatBarView.setStackViewItems([chatBarView.galleryButton, chatBarView.documentButton], forStack: .left, animated: false)
